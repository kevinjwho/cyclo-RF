function [scope]=initialize_scope(GPIB_addr)
% Initializes a scope object
%
% INPUTS
%   1. GPIB_addr - an optional agrument for the scope GPIB address
%
% OUTPUTS
%   1. scope - a scope object used to collect data from the bpm scope
%
% NOTES
% Edited July 2017 by Levon D.
% -- added a check to see if setpath already initialized the scope
%

% check if scope has already been initialized by setpath
if nargin < 1
    GPIB_addr = 8;
end

if GPIB_addr==8 % BPM scope
%ad = getad;
%try
%    scope = ad.scopes.bpm;
%    return
%catch
%    % no scope, continue on
%end
end


%DPO7254 Scope Initialization
scope.yData1=[]; 
scope.xData1=[];
scope.yData2=[]; 
scope.xData2=[];
scope.yData3=[]; 
scope.xData3=[];
scope.yData4=[]; 
scope.xData4=[];

scope.xUnits1=[];
scope.yUnits1=[];
scope.xUnits2=[];
scope.yUnits2=[];
scope.xUnits3=[];
scope.yUnits3=[];
scope.xUnits4=[];
scope.yUnits4=[];

% Instrument control variables
scope.interfaceObj = [];
scope.deviceObj = [];
scope.channelObj = [];
scope.waveformObj = [];
scope.channelObj1 = []; 
%waveformObj1 = [];



%% Instrument Connection

% Create a VISA-USB object.
scope.interfaceObj = instrfind('Type', 'visa-usb', 'RsrcName', 'USB0::0x0699::0x0401::C003131::0::INSTR', 'Tag', '');

% Create the VISA-USB object if it does not exist
% otherwise use the object that was found.
if isempty(scope.interfaceObj)
    scope.interfaceObj = visa('KEYSIGHT', 'USB0::0x0699::0x0401::C003131::0::INSTR');
else
    fclose(scope.interfaceObj);
    scope.interfaceObj = scope.interfaceObj(1);
end

fclose(scope.interfaceObj);  %temporarily close the scope to set buffersize

set(scope.interfaceObj,'InputBufferSize',2e6);
set(scope.interfaceObj,'OutputBufferSize',2e6);

fopen(scope.interfaceObj);

fprintf(scope.interfaceObj,'DAT:STOP 1000')


% Create a device object. 
scope.deviceObj = icdevice('MSO4104.mdd', scope.interfaceObj);

% Connect device object to hardware.
%connect(scope.deviceObj);

% Query property value(s).
%get1 = get(scope.deviceObj, 'InstrumentModel');
%connect(scope.deviceObj);
%if ad.bpm_scope_avg == 16
%    set(scope.deviceObj.Acquisition,'NumberOfAverages',16);
%end

%groupObj = get(scope.deviceObj, 'Waveform');

%[Y,X,YUNIT,XUNIT] = invoke(groupObj, 'readwaveform', 'channel3');


%%
%scope.channelObj1 = scope.deviceObj.Channel(1); % read from channel 1
scope.channelObj2 = scope.deviceObj.Channel(2) % read from channel 2
scope.channelObj3 = scope.deviceObj.Channel(3); % read from channel 3
scope.channelObj4 = scope.deviceObj.Channel(4); % read from channel 4

%scope.waveformObj1 = scope.deviceObj.Waveform(1);  % default waveform measurement 1
scope.waveformObj2 = scope.deviceObj.Waveform(1)  % default waveform measurement 2
scope.waveformObj3 = scope.deviceObj.Waveform(1);  % default  waveform measurement 3
scope.waveformObj4 = scope.deviceObj.Waveform(1);  % default  waveform measurement 4
























