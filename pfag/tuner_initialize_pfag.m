function [pfag] = tuner_initialize_pfag(tcpipaddr)
...
pfag.prop.period = 1/freq; % period of pulse in seconds (:PER1 xx)
pfag.prop.polarity = 0; % 0 - NORM , 1 - Flipped (:OUTP1:POL NORM/INV)
pfag.prop.amp = 4.52; % amplitude in volts (:VOLT1:AMPL 1) == 17 dBm
...
fprintf(pfag.deviceObj,':FUNC1:USER'); % this creates
fprintf(pfag.deviceObj,':FUNC1 USER'); % this outputs
fprintf(pfag.deviceObj,':FUNC2:USER'); % this creates
fprintf(pfag.deviceObj,':FUNC2 USER'); % this outputs


%fprintf(pfag.deviceObj,':FUNC SIN');
%fprintf(pfag.deviceObj, ':APPL:SIN 22.87MHz, 17dBm, 0');
